...
# --- 1. Data Retrieval ---
# Get the category and the value-to-add from storage
execute store result score #category pd_temp run data get storage productivity_decay:cmd category 1
execute store result score #value_to_add pd_temp run data get storage productivity_decay:cmd param_value 1
...
# Add the value and store it in #new_score
scoreboard players operation #new_score pd_temp = #current_score pd_temp
scoreboard players operation #new_score pd_temp += #value_to_add pd_temp
...
tellraw @s [{"text":"[Aura System]","color":"gold"},{"text":" Added ","color":"yellow"},{"score":{"name":"#value_to_add","objective":"pd_temp"},"color":"white"},{"text":" to ","color":"yellow"},{"storage":"productivity_decay:cmd","nbt":"category_name","color":"green"},{"text":". New total: ","color":"yellow"},{"score":{"name":"#new_score","objective":"pd_temp"},"color":"white"}]
